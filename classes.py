...
class Puppy:
    def __init__(self, image_file, pos, vel):
        ...
        self.vel = vel
        self.size = [int(x * 1) for x in pic.get_size()]  # x * [size rescaling]
        self.IMG_W = 110  # size of action left to right
        self.IMG_H = 138
        self.my_image = pygame.transform.scale(pic, self.size)
    ...
    def update(self, delta_t):
        ...
        self.clip = pygame.Rect(self.MARGIN + self.IMG_W * self.frame, self.side * self.IMG_H,
                                self.IMG_W, self.IMG_H)
        return
...
class Path:
    def __init__(self, pos, size):
        ...
        self.accident_id = 0
        self.cleanup_timer = 20

    # Changes object attribute if puppy is within space
    def update(self, other_rect):
        # Path space is in warning, touching the player sprite
        if self.status == 'warning' and self.rect.colliderect(other_rect) and other_rect.sprite_id == 1:
            if other_rect.cleanup:
                # What kind of accident and what items are equipped?
                # Accident id 1 for pee needs spray = item id 01
                # Accident id 2 for poop needs doggie bag = item id 04
                if self.accident_id == 1:
                    if other_rect.item1 == 1:
                        self.cleanup_timer -= 1
                    elif other_rect.item2 == 1:
                        self.cleanup_timer -= 1
                elif self.accident_id == 2:
                    if other_rect.item1 == 4:
                        self.cleanup_timer -= 1
                    elif other_rect.item2 == 4:
                        self.cleanup_timer -= 1
            # Cleanup complete, remove item from inventory
            if self.cleanup_timer <= 0:
                #other_rect.item_count -= 1
                other_rect.item_id = 00
                other_rect.cleanup = False
                self.status = 'idle'
                self.color = object_color_idle
                self.accident_id = 0
                self.pup_intersect = False
        # Path space is touching Puppy sprite
        elif self.rect.colliderect(other_rect) and other_rect.sprite_id == 2:
            self.color = object_color_warning
            self.status = 'warning'
            #Pup is here
            self.accident_id = random.randint(1, 2)
    ...
    def draw(self, scr):
        ...
        if self.pup_intersect:
            ...
            self.rect = pygame.Rect(int(self.pos.x), int(self.pos.y), self.size.x, self.size.y)
...
class ghost:
    def __init__(self, image_file, pos, vel):
        ...
        self.vel = vel
        self.size = [int(x * 1) for x in pic.get_size()]  # x * [size rescaling]
        self.IMG_W = self.size[0]  # size of action left to right
        self.IMG_H = self.size[1]
        self.my_image = pygame.transform.scale(pic, self.size)
        self.rect = pygame.Rect(self.pos.x, self.pos.y, self.IMG_W, self.IMG_H)
        # Path operations
        self.target = None
